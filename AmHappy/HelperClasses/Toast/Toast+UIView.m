...
- (CGPoint)getPositionFor:(id)point toast:(UIView *)toast {
    ...
    if([point isKindOfClass:[NSString class]]) {
        ...
        {
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - kVerticalPadding-50);
        } else if( [point caseInsensitiveCompare:@"center"] == NSOrderedSame ) {
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2);
        }else if ( [point caseInsensitiveCompare:@"Rahul"] == NSOrderedSame ) {
            return CGPointMake(self.bounds.size.width / 2, (self.bounds.size.height+300) / 2);
        }
...
- (UIView *)makeViewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    [wrapperView.layer setBorderColor:[[UIColor whiteColor] CGColor]];
    [wrapperView.layer setBorderWidth:2.0];
    
    if (kDisplayShadow) {
        [wrapperView.layer setShadowColor:[UIColor grayColor].CGColor];
        [wrapperView.layer setShadowOpacity:0.9];
        [wrapperView.layer setShadowRadius:6.0];
         [wrapperView.layer setShadowOffset:CGSizeMake(4.0, 4.0)];
    }

    [wrapperView setBackgroundColor:[[UIColor orangeColor] colorWithAlphaComponent:kOpacity]];
    ...
    if (message != nil) {
        ...
        [messageLabel setFont:[UIFont fontWithName:@"Helvetica Neue" size:15.0]];
