...
- (void)continueSendMessage:(XMPPMessage *)message withTag:(long)tag
{
	...
	NSData *outgoingData = [outgoingStr dataUsingEncoding:NSUTF8StringEncoding];
//	NSLog(@"%@",[message compactXMLString]);
//    NSLog(@"%@",[message elementsForName:@"received"]);
    if ([message elementsForName:@"received"].count) {
         NSLog(@"====ccc ddd%@",[[[[message elementsForName:@"received"] objectAtIndex:0] attributeForName:@"id"] stringValue]);
       /* NSManagedObject *array = [CoreDataUtils getObject:[DB description] withQueryString:@"idd = %@" queryArguments:@[[[[[message elementsForName:@"received"] objectAtIndex:0] attributeForName:@"id"] stringValue]]];
...
//    NSLog(@"%@",[element elementsForName:@"received"]);
//    NSLog(@"%@",[element attributeForName:@"to"]);
    NSString *ac = [element compactXMLString];
    if ([ac rangeOfString:@"delivered"].location == NSNotFound) {
       // NSLog(@"Not Deliver");
    }
    else {
        NSXMLNode *x = [element nextNode];
        NSLog(@"====ccc %@", [[x childAtIndex:1] stringValue]);
        
       NSManagedObject *array = [CoreDataUtils getObject:[DB description] withQueryString:@"idd = %@" queryArguments:@[[[x childAtIndex:1] stringValue]]];
    ...
    if ([ac rangeOfString:@"reciept"].location == NSNotFound ) {
       // NSLog(@"=====Not reciept");
    }
    else {
        NSXMLNode *x = [element nextNode];
