...
- (void)drawRect:(CGRect)rect
{
    ...
    if([USER_DEFAULTS valueForKey:@"localization"])
    {
        if([[USER_DEFAULTS valueForKey:@"localization"] isEqualToString:@"S"])
        {
            ...
            [formatter setLocale:[[NSLocale alloc] initWithLocaleIdentifier:@"es"]];
            
        }
        else if([[USER_DEFAULTS valueForKey:@"localization"] isEqualToString:@"C"])
        {
            [localization setLanguage:@"CH"];
            [formatter setLocale:[[NSLocale alloc] initWithLocaleIdentifier:@"zh-Hant"]];
   ...
   // CGContextSetFillColorWithColor(context, [UIColor whiteColor].CGColor);
    ...
    {
        if ([[NSString stringWithFormat:@"%@",[weekdays objectAtIndex:i]] isEqualToString:@"Mon"])
        {
            [weekdays replaceObjectAtIndex:i withObject:[NSString stringWithFormat:@"%@",[localization localizedStringForKey:@"Mon"]]];
        }
        else if ([[NSString stringWithFormat:@"%@",[weekdays objectAtIndex:i]] isEqualToString:@"Tue"])
        {
            [weekdays replaceObjectAtIndex:i withObject:[NSString stringWithFormat:@"%@",[localization localizedStringForKey:@"Tue"]]];
        }
        else if ([[NSString stringWithFormat:@"%@",[weekdays objectAtIndex:i]] isEqualToString:@"Wed"])
        {
             [weekdays replaceObjectAtIndex:i withObject:[NSString stringWithFormat:@"%@",[localization localizedStringForKey:@"Wed"]]];
        }
        else if ([[NSString stringWithFormat:@"%@",[weekdays objectAtIndex:i]] isEqualToString:@"Thu"])
        {
             [weekdays replaceObjectAtIndex:i withObject:[NSString stringWithFormat:@"%@",[localization localizedStringForKey:@"Thu"]]];
        }
        else if ([[NSString stringWithFormat:@"%@",[weekdays objectAtIndex:i]] isEqualToString:@"Fri"])
        {
            [weekdays replaceObjectAtIndex:i withObject:[NSString stringWithFormat:@"%@",[localization localizedStringForKey:@"Fri"]]];
        }
        else if ([[NSString stringWithFormat:@"%@",[weekdays objectAtIndex:i]] isEqualToString:@"Sat"])
        {
            [weekdays replaceObjectAtIndex:i withObject:[NSString stringWithFormat:@"%@",[localization localizedStringForKey:@"Sat"]]];
        }
        else if ([[NSString stringWithFormat:@"%@",[weekdays objectAtIndex:i]] isEqualToString:@"Sun"])
        {
            [weekdays replaceObjectAtIndex:i withObject:[NSString stringWithFormat:@"%@",[localization localizedStringForKey:@"Sun"]]];
        }
    }
    ...
    for (int i =0; i<[weekdays count]; i++) {
       ...
       // UIFont *font = [UIFont fontWithName:@"HelveticaNeue" size:12];
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth+2), 40, kVRGCalendarViewDayWidth+2, 20) withFont:FONT_Regular(12) lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
   ...
   /* //Grid dark lines
    ...
    /*
     ...
     maskPath = [UIBezierPath bezierPathWithRoundedRect:view.imgHaeder.bounds
     byRoundingCorners:(UIRectCornerTopRight| UIRectCornerTopLeft)
     cornerRadii:CGSizeMake(3, 3)];
     
     CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
     maskLayer.frame = view.imgHaeder.bounds;
     maskLayer.path = maskPath.CGPath;
     view.imgHaeder.layer.mask = maskLayer;
     */
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            if(IS_Ipad)
            {
                rectangleGrid = CGRectMake(targetX+30,targetY,50,50);
            }
            else if(IS_IPHONE_6 || IS_IPHONE_6_PLUS)
            {
                rectangleGrid = CGRectMake(targetX+7,targetY,40,40);

            }
            else
            {
                rectangleGrid = CGRectMake(targetX,targetY,45,45);
            }
           ...
           /* CGContextAddRect(context, rectangleGrid);
            ...
            CGRect borderRect = rectangleGrid;
            CGContextAddRect(context, borderRect);
            CGContextSetRGBStrokeColor(context, 1.0, 1.0, 1.0, 1.0);
            CGContextSetFillColorWithColor(context, [UIColor colorWithRed:228/255.0f green:123/255.0f blue:0/255.0f alpha:1.0].CGColor);
            ...
            {
                rectangleGrid = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth+2,kVRGCalendarViewDayHeight+2);
            }
      ...
      //  [date drawInRect:CGRectMake(targetX+2, targetY+10, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:FONT_Regular(14) lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
          [date drawInRect:CGRectMake(targetX+2, targetY+10, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont systemFontOfSize:18] lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        NSLog(@"marks date are %@",self.markedDates);
        ...
        if(IS_Ipad)
        {
            rectangle = CGRectMake(targetX+45,targetY,5,2);
        }
        else
        {
            rectangle = CGRectMake(targetX+15,targetY,5,2);
        }
...
-(id)init {
    ...
    if (self) {
        ...
        isAnimating=NO;
        self.labelCurrentMonth = [[UILabel alloc] initWithFrame:CGRectMake(20, 0, kVRGCalendarViewWidth-54, 40)];
        [self addSubview:labelCurrentMonth];
        labelCurrentMonth.backgroundColor=[UIColor clearColor];
        labelCurrentMonth.font = FONT_Heavy(15);
